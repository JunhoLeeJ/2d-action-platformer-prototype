...
const CONFIG = {
  ...
  // 숏홉(점프->공중 공격) 콤보 판정 유예 - 점프와 공격 클릭을 사람이 인지 못 할 만큼 짧은 간격으로
  // 거의 동시에 누를 때, 입력 이벤트 처리 순서가 미세하게 흔들려서 "공격 클릭이 점프보다 아주 살짝
  // 먼저" 처리되면 그 스윙이 지상 공격으로 확정되어버려(그 순간 아직 onGround가 true였으므로) 곧바로
  // 이어지는 점프가 지상 공격 전용 조작 잠금에 막혀 콤보 자체가 깨지는 문제가 있었다. 지상 스윙이
  // 시작된 지 이 시간(sec) 이내에 점프가 들어오면 그 스윙을 공중 공격으로 전환해 점프가 정상적으로
  // 먹히게 해준다(updatePlayer의 canConvertToShortHop 참고) - 이 시간을 넘겨서 점프가 들어오면(플레이어가
  // "공격부터 하고 한참 뒤에 점프를 눌렀다"고 인지할 시점) 절대 전환하지 않고 원래대로 막는다. 값을
  // 일부러 작게 잡아 "체감 가능한 콤보 보정"이 아니라 "입력 순서 흔들림만 봐주는" 수준으로 제한함.
  GROUND_ATTACK_JUMP_CONVERSION_WINDOW: 0.06,
