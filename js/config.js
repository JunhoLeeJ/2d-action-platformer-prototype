...
const CONFIG = {
  ...
  // --- 적 (배경 몬스터 A/B - 1층 구역 4, "전투 대상 아님" 구간 전용) ---
  // 적 A는 "재스킨한 체이서" - 전투 능력(감지/추적/공격) 자체는 makeChaser를 그대로 재사용하고
  // (makeMimeA 참고), 새로 필요한 건 체력값과 유휴 손짓 애니메이션 파라미터뿐이라 이 둘만 별도 CONFIG로 둠.
  // "배경 몬스터"라는 성격은 AI가 약해서가 아니라 레벨 디자인(이 구역엔 wallGates가 없어 억지로
  // 싸울 필요가 없음)으로 만든다 - CLAUDE.md에 문서화된 "이미 있는 매커니즘 재사용" 원칙을 따름.
  MIME_A_MAX_HP: 2,
  ...
  // 적 B(웅얼거림) - 완전 수동형(감지/공격 없음), 몸통 색이 옅게 밝아졌다 어두워지는 pulse만 있음.
  MIME_B_MAX_HP: 1,
